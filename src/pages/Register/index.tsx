...
import { Container, ContainerRegister, FormContainer, ImgBackground, InputForm } from "./styles";
...
const registerFormSchema = z.object({
  ...
  birthDate: z.date({required_error: "O campo data de nascimento é obrigatório", invalid_type_error: "Formato de dado inválido"}).min(new Date("1900-01-01"), {message: "Data inválida"}).max(new Date(), {message: "Data inválida"}),
  email: z.string().email({message: "Formato de email inválido"}).min(1, {message: "O campo email é obrigatório"}),
  password: z.string().nonempty({message: "O campo senha é obrigatório"}),
  confirmPassword: z.string().nonempty({message: "O campo confirmar senha é obrigatório"}),
})
...
export function Register() {
  ...
  function handleValidateConfirmPasswordChange(event: ChangeEvent<HTMLInputElement>) {
    event.target.setCustomValidity("");
          ...
          <fieldset>
            ...
            <InputForm>
              <input
                ...
                className={
                  Object.keys(formErrors)[0] === "userName" ? 'invalid-input' : ''
                }
              />
            ...
            <InputForm>
              <input
                ...
                className={
                  Object.keys(formErrors)[0] === "birthDate" ? 'invalid-input' : ''
                }
              />
            ...
            <InputForm>
              <input
                ...
                className={
                  ...
                  Object.keys(formErrors)[0] === "confirmPasswordInvalid" ? 'invalid-input' : ''
                }
              />
            ...
            { Object.keys(formErrors)[0] && <p className="invalidInput">{formErrors[Object.keys(formErrors)[0]]}</p> }
